...
public class TestClass {
	...
	public static void main(String[] args) {
		...
		Journeys journeys = Parser.getJourneys(searchURL); // Denna gör om infon
															// från XML taggarna
															// vi sökt fram i
															// seachURL ovan
															// till Strings och
		...
		for (Journey journey : journeys.getJourneys()) {
		 ...
		 //skapar en ny ArrayList för Stationer som börjar med "Malm"
		 
		 searchStations.addAll(Parser.getStationsFromURL("Malm")); 
		 //lägger till alla sökresultat i den ArrayList som just skapats
		 for (Station s: searchStations){
		 System.out.println(s.getStationName() +" number:"
		 +s.getStationNbr());
		 }
		
		 //Söker efter avgångar från en viss station
